...
export default function ValueCols(props) {
  return (
    <section className="flex flex-col items-center justify-center pb-20">
      <Fade bottom cascade>
        <div
          className={`container mx-auto px-8 grid md:grid-cols-${props.pillars.length} gap-12 -mt-24 text-center mb-12`}
        >
          {props.pillars.map((p, i) => {
            return (
              <div key={i} className="text-center flex flex-col items-center">
                <div className="bg-gray-200 rounded-full relative mb-12 w-2/3 max-w-xs">
      ...
      <Link to="/shop" className="btn-small leading-normal text-center">
        {props.text}
      </Link>
    </section>
  )
