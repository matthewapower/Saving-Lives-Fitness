...
const Header = props => {
  ...
  useEffect(() => {
    if (props.navOpen) {
      ...
      setLogoPos("translateY(90%) scale(1.6)")
    }
  }, [props.navOpen, setLogoPos])

  return (
    <>
      <header className="grid absolute top-0 inset-x-0 items-center justify-center grid-cols-1 md:grid-cols-5">
        ...
        <Link
          to="/"
          className="p-4 z-50 md:order-2 mx-auto w-1/2 md:w-full"
          css={css`
            transform: ${logoPos};
            max-width: 400px;
